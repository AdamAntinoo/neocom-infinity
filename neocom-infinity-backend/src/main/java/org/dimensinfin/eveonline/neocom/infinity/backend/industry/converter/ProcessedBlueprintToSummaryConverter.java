...
//		final double costIndex = (input.getOutput().getMarketPrice() * input.getOutput().getQuantity()) * 0.9 / manufactureCost;
		return new ProcessedBlueprintSummary.Builder()
				...
				.withBlueprint( input.getBlueprintItem() )
				.withOutput( input.getOutputItem() )
				.withTradeStation( (Station) input.getLocation()  )
				.withManufactureCost( input.getManufactureCost() )
				.withCostIndex( input.getOutputCost() / input.getManufactureCost() )
				.build();
	}
}