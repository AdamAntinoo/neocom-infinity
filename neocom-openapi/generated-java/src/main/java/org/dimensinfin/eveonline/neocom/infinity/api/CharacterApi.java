...
public interface CharacterApi {
    ...
    /**
     * GET /api/v3/neocom/character/blueprints/session/{sessionid} : Get the list of blueprints for a character under a session to be able to cache the data.
     * Get the list of blueprints for a character. Add a session identifier that will differentiate requests from different characters and then be able to cache the results for each different requester. Session is an architecture element that has no application functionality but to identify elements to cache. Session is generated by the frontend on each new application initialization. 
     *
     * @param sessionid The session identifier to be used to cache the blueprints. (required)
     * @param NEOCOM_TOKEN The access token to be used for authorization. This token will contain information to locate the credential to be used for data extraction. (required)
     * @return List of blueprints obtained and processed successfully. The new list is returned to the client. (status code 200)
     *         or Unauthorized. The current active access token is not present. The character identified thus cannot be accessed and the credential cannot be found. (status code 401)
     *         or Forbidden. The credential information is not valid to create a new access token and the authorization request is forbidden. (status code 403)
     * @deprecated
     */
    ...
    @Operation(
        ...
        description = "Get the list of blueprints for a character. Add a session identifier that will differentiate requests from different characters and then be able to cache the results for each different requester. Session is an architecture element that has no application functionality but to identify elements to cache. Session is generated by the frontend on each new application initialization. ",
        tags = { "Character" },
        responses = {
            @ApiResponse(responseCode = "200", description = "List of blueprints obtained and processed successfully. The new list is returned to the client.", content = {
    ...
    @RequestMapping(
        method = RequestMethod.GET,
        value = "/api/v3/neocom/character/blueprints/session/{sessionid}",
        produces = "application/json"
    )
    ResponseEntity<List<Object>> getCharacterBlueprints(
        @Parameter(name = "sessionid", description = "The session identifier to be used to cache the blueprints.", required = true, in = ParameterIn.PATH) @PathVariable("sessionid") String sessionid,
        @NotNull @Parameter(name = "NEOCOM-TOKEN", description = "The access token to be used for authorization. This token will contain information to locate the credential to be used for data extraction.", required = true, in = ParameterIn.HEADER) @RequestHeader(value = "NEOCOM-TOKEN", required = true) String NEOCOM_TOKEN
    );
