...
When('the endpoint {string} is activated from request', async function (this: NIN01World, endpoint: string) {
    ...
    const request: string = 'http://localhost:3000/nin/v1/character/123/miningoperations'
    ...
    const sut = this.httpService.get<V2MiningOperation[]>(request, config)
    ...
    // console.log(sut)
    await sut.subscribe(data => {
        // console.log('then.data->' + data)
        this.miningActionsResponse = data
    })
})
Then('there is a Token cookie', function (this: NIN01World) {
    const tokenFuture: Option<string> = this.authenticationService.getCurrentToken()
...
Then('the received token is valid', function (this: NIN01World) {
    expect(this.authenticationService.validateToken(this.token)).toBeTruthy
})
