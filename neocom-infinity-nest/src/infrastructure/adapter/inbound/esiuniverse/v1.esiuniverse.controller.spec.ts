...
describe('CONTROLLER V1EsiUniverseController [Module: Infrastructure.Adapters]', () => {
	...
	beforeEach(async () => {
		const moduleRef = await Test.createTestingModule({
			...
			providers: [
				{
					provide: GetTypeInformationUseCase,
					useValue: {
				...
				{
					provide: GetSpaceLocationUseCase,
					useValue: {
						esiGetSpaceLocation: (locationId: number) => {
							return new Promise(resolve => {
								resolve(
									new V1SpaceLocationDto({
	...
	describe('Endpoints phase', () => {
		test('when a request for esi type arrives we get a valid response', async () => {
			...
			const sut: V1EsiTypeDto = await controller.esiGetTypeInformation({ typeId: typeId }).then((response: V1EsiTypeDto) => {
		...
		test('when a request for market data arrives we get a valid response', async () => {
			...
			const sut: V1MarketDataDto = await controller.esiGetMarketData(input).then((response: V1MarketDataDto) => {
		...
		test('when a request for space location data arrives we get a valid response', async () => {
			...
			const sut: V1SpaceLocationDto = await controller.esiGetLocation({ locationId: locationId }).then((response: V1SpaceLocationDto) => {
