...
Then('the page {string} is activated', function (symbolicName: string) {
	...
	cy.log('>[Route]> ' + route)
	cy.visit(route)
	cy.get('app-root').find(tag).as('target-page').as('target-panel').as('target').should('exist')
})
Then('the route {string} is activated', function (symbolicName: string) {
	const tag = supportService.translateTag(symbolicName) // Do name replacement
	const route = supportService.translateRoute(symbolicName) // Get the route for this page name
	cy.log('>[Translation]> ' + symbolicName + ': ' + tag)
	cy.log('>[Route]> ' + route)
...
Then('the page {string} has {int} panels', function (symbolicName: string, panelCount: number) {
	const tag = supportService.translateTag(symbolicName) // Do name replacement
	cy.get('app-root').find(tag).find('.panel').should('have.length', panelCount)
})
Then('the page {string} has {int} sections', function (symbolicName: string, sectionCount: number) {
	const tag = supportService.translateTag(symbolicName) // Do name replacement
	cy.get('app-root').find(tag).find('.section').should('have.length', sectionCount)
})
...
Then('the section {string} has {int} panels', function (sectionName: string, panelCount: number) {
	...
	cy.get('app-root')
		.find('[cy-section-name="' + sectionName + '"]')
		.find('.panel')
...
Then('the loading panel shows {string}', function (loadingMessage: string) {
	cy.get('@target-page').find('.loading-message').contains(loadingMessage, { matchCase: false })
})
...
Given('the target is the panel of type {string}', function (symbolicName: string) {
	const tag = supportService.translateTag(symbolicName) // Do name replacement
	cy.log('>[translation]> ' + symbolicName + ' -> ' + tag)
	cy.get('@target-page').find(tag).as('target-panel').as('target')
})
...
Then('the target has the title {string}', function (title: string) {
	cy.get('@target').get('.panel-title').contains(title, { matchCase: false })
})
...
Then('field named {string} has contents {string}', function (fieldName: string, fieldValue: string) {
	cy.get('@target').within($item => {
		cy.get('.field')
			.find('[cy-field-value="' + fieldName + '"]')
			.contains(fieldValue, { matchCase: false })
...
When('the Feature with label {string} is clicked the destination is the Page {string}', function (label: string, destinationTag: string) {
	const destination = supportService.translateTag(destinationTag)
	cy.get('@target-page').find(supportService.translateTag('feature-button')).contains(label, { matchCase: false }).parent().scrollIntoView().click()
