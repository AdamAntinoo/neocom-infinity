...
import { ProcessedBlueprint } from '../../../industry/domain/V1ProcessedBlueprint.domain'
...
export class ProcessedBlueprintDto {
	...
	public transform(universeService: UniverseService): ProcessedBlueprint {
		...
		if (this.blueprintType) {
			universeService.apiv1_GetUniverseType(this.blueprintType).subscribe((universeType: EsiType) => {
		...
		blueprint.bom = []
		for (const resource of this.bom) blueprint.bom.push(new Resource(resource))
		return blueprint
	}
}
